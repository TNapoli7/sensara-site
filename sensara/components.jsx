  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        // Progress from "top of section hits bottom of viewport" to "bottom of section hits top"
...
/* ————————————————— Reveal wrapper ————————————————— */
function Reveal({ children, delay = 0, className = '', as = 'div', clip = false }) {
  const [ref, inView] = useReveal();
  const Tag = as;
  const cls = clip ? `clip-reveal ${inView ? 'in' : ''}` : `reveal ${inView ? 'in' : ''}`;
...
/* ————————————————— Car schematic SVG (original, simple line art) ————————————————— */
function CarSchematic({ hover, setHover }) {
  ...
  return (
    <div className="relative w-full" style={{aspectRatio:'16/9'}}>
      {/* Background suede panel */}
      <div className="absolute inset-0 suede-bg" />
      <div className="absolute inset-0 grid-lines" />

      {/* Abstract side-view car silhouette (original line art) */}
      <svg viewBox="0 0 800 450" className="absolute inset-0 w-full h-full">
        ...
        {/* Car body — abstract side profile */}
        ...
        {/* Connector lines from hotspots to labels (appear on hover) */}
      </svg>
...
Object.assign(window, { useReveal, useSectionProgress, useScrollY, Eyebrow, Reveal, SuedeTile, Hotspot, CarSchematic });
